...
class LLM:
    ...
    def __init__(self, ml_manager: MLManager, X: pd.DataFrame) -> None:
        self.model = ml_manager.model
        self.scaler = ml_manager.scaler
        self.X = pd.DataFrame(
...
10. Finish with a short overall summary.
    ...
    def explain_student(self, student_id: int, df: pd.DataFrame) -> str:
        idx = df.loc[df["id_student"] == student_id].index[0]
        sample = self.X.loc[idx]
        shap_values = self.explainer(sample.to_frame().T)
        prediction = self.model.predict(sample.to_frame().T)[0]
